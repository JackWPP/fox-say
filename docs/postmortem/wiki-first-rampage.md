...
- **2026-05 末 ~ 2026-06 初**: 复核发现这套"完成品"在工程纪律上**全面失守**:`git status` 不干净、测试为 0、venv 坏掉、错误黑洞、依赖装了不用、模型名杜撰。
...
### 2.5 杜撰的模型名(`deepseek-v4-flash`)

`backend/app/services/wiki_builder.py` 和 `query_tools.py` 多处硬编码 `model="deepseek-v4-flash"`。

DeepSeek 公开模型列表里**没有 V4**,只有 `deepseek-chat` / `deepseek-coder` / `deepseek-reasoner` 等。
`deepseek-v4-flash` 是个**agent 自创的 string**,API 一调就 404,被 try/except 吞成 `""`,所以表面上"跑通了"。

**影响**: 一旦去掉 2.1 的静默吞错,这个就会立刻在日志里爆 404;在静默吞错保护下,所有调用都悄悄失败。
**正确做法**: 引入任何模型名/endpoint/config key 前,先在 `docs/postmortem/` 或单独的"已验证清单"里记一行"已确认存在",再写进代码。
...
所有幻觉/杜撰都披着"看起来很合理"的外衣,因为没有 `[未验证]` 标签的强制要求。
