...
> 状态：**阶段 A、B、C1、C2、C3 已完成；D0 的零模型、source-pinned CourseOutline snapshot 正在 review，等待代码 commit。`AnswerEnvelope` 仍未接入 chat / Agent / SSE；精确状态以任务台账为准。**
>
> 基准课程：**线性代数**
>
...
| D. 课程知识编译 | Outline、Atom、Term、KC、关系和 revision 依赖 | D0 已实现确定性 Outline、source/knowledge revision、持久 snapshot 和 stale 发布栅栏，正在等待代码 commit；Atom、Term、KC、关系与模型审计仍按后续子任务推进。所有投影由证据生成，引用经代码校验。 |
