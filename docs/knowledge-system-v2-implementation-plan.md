...
1. `material_index`：解析、fragment、embedding、向量写入的材料级幂等工作。
2. `course_compile`：基于一个明确 source revision 的课程级知识投影工作。
...
| B. 可恢复材料索引 | 受控 worker、`material_index` job、fragment 持久化和稳定 Qdrant ID | 重启后材料索引不会丢；不再裸起关键任务。 |
| C. 证据检索与状态 | `EvidenceRef`、fragment preview、KnowledgeStatus、混合检索 | 原文问答可引用，UI 能显示真实状态。 |
| D. 课程知识编译 | Outline、Atom、Term、KC、关系和 revision 依赖 | 所有投影由证据生成，引用经代码校验。 |
