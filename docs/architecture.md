...
Document parsing: markitdown (统一入口, 输出 Markdown) → pdfplumber/python-pptx (本地 fallback) → MinerU (PDF/图片云端 OCR fallback)
  支持格式: PDF / PPT / Word / HTML / 图片(PNG/JPG) / 文本(TXT/MD)
...
Course creation
  ...
  → async parsing 三层 fallback (markitdown → pdfplumber/python-pptx → MinerU OCR)
...
- markitdown 失败时回退到 pdfplumber/python-pptx (Layer 2), PDF/图片最后走 MinerU 云端 OCR (Layer 3)。
- MinerU 仅在解析内容过薄 (len < 200) 时作为 fallback 触发, 不再是"成功后增强"。
