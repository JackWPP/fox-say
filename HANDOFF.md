...
**当前进度**:从 PRD 46% → **~55%**。脚手架就位,但还没在真实《线代》材料上跑过。
...
```
当前分支:   feature/pr0-schema-contracts
HEAD:       80169f4add6a2cf8b0fb361ac20c4372db0abebb
远端:       origin  → https://github.com/JackWPP/fox-say.git
origin/main: 9fae15e (PR0 + 之后 HEAD 7 个 commit 未 push)
working tree: 干净(git status 无输出)
```

### 5.2 完整 commit 历史(从 PR0 起)

```
80169f4 (HEAD) Add visual verification script for Q&A tab functionality
ab9c9cb wip: catch up pre-existing frontend chat + agent.py from earlier session
056b532 merge: Line C — 知识图谱可视化 MVP
068aca2 merge: Line B — 评测脚手架 + 30 题 pilot
dd12d8c merge: Line A — prereq ETL 脚手架
c7d5b0a feat(line-b): 评测脚手架 + 30 题 pilot(MVP)
59242ca feat(line-a): prereq ETL 脚手架(MVP)
d69a0f2 feat(line-c): 知识图谱可视化 MVP(后端 + 前端骨架)
9fae15e feat(judge): 加 fast judge + reranker 模型配置  ← PR0 follow-up
d191c61 feat(schema): PR0 锁定三线并行的共享 contract     ← PR0 main
```
...
`scripts/qa_visual_check.py` — playwright 真实交互验证脚本(已 commit,但用户没说它跟三线集成关系)。
...
4. **修 `ChapterWiki.overview` 空字符串** — 是 P1,Drawer 才有内容显示
