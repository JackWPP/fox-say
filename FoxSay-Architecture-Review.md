...
> ⚠️ 部分内容已过时 — 2026-06-29 更新，HEAD e437ab3。以下 P0 问题已修复，详见各节标注。
...
**4. Agent + 7 工具集设计清晰。**（✅ 已更新：实际为 11 工具，新增 4 个动态 Skill）

- ReAct 循环 max 3 轮（✅ 已更新：MAX_ROUNDS 已调整为 8），有 DSML 防御层（处理 DeepSeek 模型输出非法 tool-call 语法的问题）
...
| `agent.py` | `backend/app/services/agent.py` | ~320 | Agent ReAct 主循环 + 7 工具定义（✅ 已更新：实际 11 工具） + DSML 防御 |
...
| `CitationCard.tsx` | `frontend/src/features/course/` | 引用卡片（✅ 已修复：source-preview endpoint + 浮动预览已实现） |
