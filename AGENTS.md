...
运行时课程 job 与工程重构任务是两条不同的持久化链路，任何 Agent 都不得混淆它们。

- **运行时事实源**:`knowledge_jobs`（及其关联的 revision、产物和 `KnowledgeStatus`）是课程处理任务的唯一状态事实源。SSE、日志、聊天消息和内存中的 agent plan 都只是一种通知或观察，不能改变或代替 job 状态。
- **工程事实源**:[`docs/knowledge-system-v2-task-registry.md`](docs/knowledge-system-v2-task-registry.md) 是 V2 重构任务、依赖、验收证据和交接状态的唯一台账。口头分工、子 Agent 回报或临时 TODO 不构成已领取、已完成或已验收的任务记录。
- **单一协调者**:当前实施协调者负责领取/拆分/排序任务、指定互斥的文件范围，并串行更新台账；执行 Agent 先阅读台账再开始，仅回报实际修改、验证、风险和 commit，不得擅自把共享任务标为完成或覆盖其他 Agent 的范围。
- **开始与交接**:每项工作必须引用既有 `task_id`；新发现的工作先以带依赖、文件范围、成本/时延影响和验收方式的任务加入台账，再实现。交接时必须留下当前 revision、未完成步骤、失败原因和下一条可执行命令，不能以“处理中”作为终态。
- **完成门槛**:文档状态只能由对应阶段的可重复验证与 commit 佐证。代码在未通过约定检查或未提交前最多是 `review`/`active`，不得标为 `complete`；运行时 job 同样只有其所需产物在目标 revision 可验证后才可 `succeeded`/`ready`。
- **并发纪律**:派发前声明允许修改的路径；两个 Agent 不得并行写同一业务文件。发现 dirty worktree、状态词汇或契约不一致时，停止扩展该范围，报告协调者并在台账记录修复任务。
- **成本与线性代数门槛**:涉及模型、VLM、embedding 或重编译的任务必须记录预算和等待时间观测。V2 的下一阶段是否推进，以台账中线性代数验收项及其证据为准，而不是“模型已返回文本”。
